...
module.exports = class SubscriptionConnection {
  ...
  async handleGQLStart (data) {
    ...
    if (typeof result[Symbol.asyncIterator] === 'function') {
      ...
      // shit happened
      // errorHack.emit('reconnect');
      this.sendMessage(GQL_ERROR, data.id, result)
      this.close();
    }
  }
  ...
  handleGQLStop(data) {
    ...
    Object.values(this.clients).forEach((client) => {
      // currently no matter by user or not
      client.unsubscribe(data.id, true);
    });
